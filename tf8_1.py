...
#第二层
weight2=tf.Variable(np.random.rand(inputX.shape[1],4))
bias2=tf.Variable(np.random.rand(inputX.shape[1],4))
y2_=tf.matmul(x1,weight2)+bias2

y=tf.placeholder(tf.float64,[None,1])

loss=tf.reduce_mean(tf.reduce_sum(tf.square((y2_-y)),reduction_indices=[1])) #刚开始少了一个括号
